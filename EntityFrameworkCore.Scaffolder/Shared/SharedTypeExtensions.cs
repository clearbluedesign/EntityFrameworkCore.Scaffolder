...
namespace ClearBlueDesign.EntityFrameworkCore.Scaffolder.Shared
{
	internal static class SharedTypeExtensions
	{
		...
		public static IEnumerable<MemberInfo> GetMembersInHierarchy(this Type type)
		{
			do
			{
				...
				foreach (PropertyInfo propertyInfo in type.GetRuntimeProperties().Where(pi => !(pi.GetMethod ?? pi.SetMethod).IsStatic))
				{
					yield return propertyInfo;
				}

				foreach (FieldInfo fieldInfo in type.GetRuntimeFields().Where(f => !f.IsStatic))
				{
...
#pragma warning restore IDE0034 // Simplify 'default' expression
		...
		public static object GetDefaultValue(this Type type)
		{
			...
			// for all value types.
			return _commonTypeDictionary.TryGetValue(type, out object value)
				? value
